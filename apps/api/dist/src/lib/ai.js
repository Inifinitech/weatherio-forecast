...
function fallbackActions(farm, w) {
    ...
    if (level === 'Act Now') {
        ...
        if (tomorrowRain > 10)
            actions.push('Schedule any field operations for this morning, before rain arrives');
        actions.push('Keep a watch on the forecast — conditions may escalate');
    }
    else {
        if (maxTemp > 28)
            actions.push('Maintain adequate irrigation — temperatures are elevated');
        if (rain === 0 && (tomorrow?.precip_mm ?? 0) === 0)
            actions.push('Good window for field operations and spraying');
        actions.push('Continue regular monitoring — conditions are currently stable');
    }
    if (farm.cropType.toLowerCase().includes('tea')) {
        if (level !== 'Normal')
            actions.push('Tea flush quality is sensitive to waterlogging — prioritize drainage in Block rows');
    }
    if (farm.cropType.toLowerCase().includes('maize')) {
        if (level === 'Act Now')
            actions.push('Maize at flowering/tasseling stage is vulnerable to lodging — stake tall plants');
    }
    return {
        riskLevel: level,
        riskReason: reason,
...
async function callGroq(farm, w) {
...
What are the top field actions for this farm in the next 24 hours?`.trim();
    ...
    return {
        riskLevel: (['Normal', 'Watch', 'Act Now'].includes(parsed.riskLevel ?? '') ? parsed.riskLevel : 'Normal'),
...
export async function getAiActions(farm, weather) {
