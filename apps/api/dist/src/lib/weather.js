...
function normalizeLiveWeather(raw, fallbackLat, fallbackLon) {
    const hourly = raw.hourly ?? [];
    const daily = raw.daily ?? [];
    const avgHumidity = hourly.length > 0
        ? Math.round(hourly.reduce((sum, h) => sum + toNum(h.humidity, 0), 0) / hourly.length)
        : 0;
...
function mockWeather(lat, lon) {
    ...
    return {
        ...
        ai_summary: 'Expect wet conditions through the week. Heavy rain Sunday and Tuesday creates waterlogging risk for low-lying fields. Plan harvest windows around dry periods Wednesday through Friday. Monitor drainage channels before each rain event.',
    };
}
function mockUsage() {
    return {
...
export async function getWeather(lat, lon, days = 7) {
    ...
    if (!res.ok)
        throw new Error(`WeatherAI ${res.status}: ${await res.text()}`);
...
export async function analyzeTreeImage(formData) {
    if (isMockMode()) {
    ...
    return {
        ...
        tree_health: {
            healthy: Number(raw.tree_health?.healthy ?? 0),
            needs_care: Number(raw.tree_health?.needs_care ?? 0),
            needs_replacement: Number(raw.tree_health?.needs_replacement ?? 0),
        },
        tree_species_guess: raw.tree_species_guess ?? null,
        observations: Array.isArray(raw.observations) ? raw.observations : [],
        recommendations: Array.isArray(raw.recommendations) ? raw.recommendations : [],
        original_image_url: raw.original_image_url ?? null,
        overlay_image_url: raw.overlay_image_url ?? null,
    };
}
